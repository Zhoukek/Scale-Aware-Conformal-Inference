...
from plotting_utils import moving_average, plot_time_series, longest_true_sequence, plot_multi_time_series
...
def old_plot_everything(coverages_list, sets_list, titles_list, y, alpha, window_start, window_end, window_loc, coverage_inset, set_inset, miscoverage_scatterplot, savename, model_name, datetimes=None):
    fig, axs = plt.subplots(nrows=2, ncols=len(coverages_list), figsize=(10 * len(coverages_list), 6), sharex=True, sharey=False)
    # plot_time_series(fig, axs[0,:], coverages_list, window_start, window_end, window_loc, False, y, "#138085", coverage_inset, False, datetimes, hline=1-alpha )  # baselines
    plot_time_series(fig, axs[0,:], coverages_list, window_start, window_end, window_loc, False, y, "#756BB1", coverage_inset, False, datetimes, hline=1-alpha )  # ECIs
    # plot_time_series(fig, axs[1,:], sets_list, window_start, window_end, window_loc, True, y, "#EEB362", set_inset, miscoverage_scatterplot, datetimes)  # baselines
    plot_time_series(fig, axs[1,:], sets_list, window_start, window_end, window_loc, True, y, "#31A354", set_inset, miscoverage_scatterplot, datetimes)  # ECIs
...
def plot_everything(coverages_list, sets_list, titles_list, y, alpha, window_start, window_end, window_loc, coverage_inset, set_inset, miscoverage_scatterplot, savename, model_name, datetimes=None):
    ...
    # plot_time_series(fig, axs[0,:], coverages_list, window_start, window_end, window_loc, False, y, "#138085", coverage_inset, False, datetimes, hline=1-alpha )  # baselines
    plot_multi_time_series(fig, axs[0], coverages_list, window_start, window_end, window_loc, False, y, "#756BB1", coverage_inset, False, datetimes, hline=1-alpha )  # ECIs
    # plot_time_series(fig, axs[1,:], sets_list, window_start, window_end, window_loc, True, y, "#EEB362", set_inset, miscoverage_scatterplot, datetimes)  # baselines
    plot_multi_time_series(fig, axs[1], sets_list, window_start, window_end, window_loc, True, y, "#31A354", set_inset, miscoverage_scatterplot, datetimes)  # ECIs
    
    axs[0].set_ylabel('Coverage', fontsize=15) # 20
    axs[1].set_ylabel('Sets', fontsize=15)

    # Set titles for the first plot
    axs[0].set_title('Coverage', fontsize=15)
    axs[1].set_title('Sets', fontsize=15)

    # Y-axis formatting for coverage
    ...
    # Y-axis formatting for sets
    ymin, ymax = axs[1].get_ylim()
    axs[1].set_ylim([ymin, ymax + 0.1 * np.abs(ymax)])
    ...
    fig.add_subplot(111, frameon=False)
    plt.tick_params(labelcolor='none', which='both', top=False, bottom=False, left=False, right=False)
    plt.xlabel("Time", fontsize=15, labelpad=10)  # fontsize=20, original labelpad=40
    os.makedirs('./plots/1v1/' + model_name, exist_ok=True)
    plt.savefig('./plots/1v1/' + model_name + "/" + savename + '.pdf', bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--key3', help='Third key for time series data extraction.')
    # parser.add_argument('--lr3', help='Learning rate associated with third key.', type=float)
    # parser.add_argument('--key4', help='Forth key for time series data extraction.')
    # parser.add_argument('--lr4', help='Learning rate associated with forth key.', type=float)
    ...
    for model_name in model_names:
        ...
        # sets3 = [np.stack(data[args.key3][args.lr3]['sets'])[T_burnin+1:,0], np.stack(data[args.key3][args.lr3]['sets'])[T_burnin+1:,1]]
        # sets4 = [np.stack(data[args.key4][args.lr4]['sets'])[T_burnin+1:,0], np.stack(data[args.key4][args.lr4]['sets'])[T_burnin+1:,1]]
        ...
        # covered3 = moving_average(((y >= sets3[0]) & (y <= sets3[1])).astype(float), args.coverage_average_length)[args.coverage_average_burnin:]
        # covered4 = moving_average(((y >= sets4[0]) & (y <= sets4[1])).astype(float), args.coverage_average_length)[args.coverage_average_burnin:]
        ...
        # sets3 = [s[args.coverage_average_burnin:] for s in sets3]
        # sets4 = [s[args.coverage_average_burnin:] for s in sets4]
        y = y[args.coverage_average_burnin:]

        # Calculate the set sizes
        sizes1 = sets1[1] - sets1[0]
        sizes2 = sets2[1] - sets2[0]
        # sizes3 = sets3[1] - sets3[0]
        # sizes4 = sets4[1] - sets4[0]
        ...
        # time_series3 = pd.Series(covered3)
        # time_series4 = pd.Series(covered4)

        window_start = args.window_start
        window_end = args.window_start + args.window_length

        # savename = datasetname + '_' + model_name + '_' + args.key1 + '_lr' + str(args.lr1) + '_' + args.key2 + '_lr' + str(args.lr2) + args.key3 + '_lr' + str(args.lr3) + '_' + args.key4 + '_lr' + str(args.lr4) + '_window' + str(args.window_length) + '_start' + str(args.window_start) + str(args.coverage_inset) + str(args.set_inset)
        savename = 'multi' + '_' + datasetname + '_' + model_name + '_' + args.key1 + '_lr' + str(args.lr1) + '_' + args.key2 + '_window' + str(args.window_length) + '_start' + str(args.window_start) + str(args.coverage_inset) + str(args.set_inset)

        # Call the plot_time_series function to plot the data
        # plot_everything([time_series1, time_series2, time_series3, time_series4], [sets1, sets2, sets3, sets4], [method_title_map[args.key1], method_title_map[args.key2], method_title_map[args.key3], method_title_map[args.key4]], y, alpha, window_start, window_end, args.window_loc, args.coverage_inset, args.set_inset, args.miscoverage_scatterplot, savename, model_name, datetimes=data['data'].index[T_burnin+1:][args.coverage_average_burnin:])
