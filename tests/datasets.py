...
def load_dataset(name):
    ...
    if name == "M4":
        ...
        first_item = df['item_id'].unique()[0]
        df = df[df['item_id'] == first_item]  
        df = df.rename(columns={'target': 'y'})
        data = df[['timestamp', 'y']]  
        data = data.melt(id_vars=['timestamp'], value_name='target')
        data = data.rename(columns={'variable': 'item_id'})
    if name == "changepoint":
        df = pd.read_csv("./datasets/changepoint.csv")
        df.rename({'timestamp': 'timestamp', 'y': 'y'}, axis='columns', inplace=True)
        # df = df.drop("Unnamed: 0", axis='columns')
        data = df.melt(id_vars=['timestamp'], value_name='target')
        data.rename({'variable': 'item_id'}, axis='columns', inplace=True)
    if name == "distribution_drift":
        df = pd.read_csv("./datasets/distribution_drift.csv")
        df.rename({'timestamp': 'timestamp', 'y': 'y'}, axis='columns', inplace=True)
        # df = df.drop("Unnamed: 0", axis='columns')
        data = df.melt(id_vars=['timestamp'], value_name='target')
        data.rename({'variable': 'item_id'}, axis='columns', inplace=True)
    if name == "heavy-tailed":
        df = pd.read_csv("./datasets/heavy-tailed.csv")
        df.rename({'timestamp': 'timestamp', 'y': 'y'}, axis='columns', inplace=True)
        # df = df.drop("Unnamed: 0", axis='columns')
        data = df.melt(id_vars=['timestamp'], value_name='target')
        data.rename({'variable': 'item_id'}, axis='columns', inplace=True)
    if name == "variance_changepoint":
        df = pd.read_csv("./datasets/variance_changepoint.csv")
        df.rename({'timestamp': 'timestamp', 'y': 'y'}, axis='columns', inplace=True)
        # df = df.drop("Unnamed: 0", axis='columns')
        data = df.melt(id_vars=['timestamp'], value_name='target')
        data.rename({'variable': 'item_id'}, axis='columns', inplace=True)
    if name == "extreme_drift":
        df = pd.read_csv("./datasets/extreme_drift.csv")
        df.rename({'timestamp': 'timestamp', 'y': 'y'}, axis='columns', inplace=True)
        # df = df.drop("Unnamed: 0", axis='columns')
        data = df.melt(id_vars=['timestamp'], value_name='target')
        data.rename({'variable': 'item_id'}, axis='columns', inplace=True)
