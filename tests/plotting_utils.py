...
def plot_multi_time_series(fig, ax, time_series_list, window_start, window_end, window_loc, sets, y, color, inset, miscoverage_scatterplot, datetimes, hline=None, label=None):
    colors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728"]
    labels = ["ECI", "RCI", "Method 3", "Method 4"]  # Customize these labels as needed

    # Create a list of time series with only finite values
    if not sets:
        ts_list_finite = [np.where(np.isfinite(time_series), time_series, np.nan) for time_series in time_series_list]
    else:
        ts_list_finite = [
            [
                np.where(np.isfinite(time_series[0]) & np.isfinite(time_series[1]), time_series[0], np.nan),
                np.where(np.isfinite(time_series[0]) & np.isfinite(time_series[1]), time_series[1], np.nan)
            ]
    ...
    for i, time_series in enumerate(ts_list_finite):
        color = colors[i]
        
        # Use seaborn to plot the time series on the ax
        if not sets:
            sns.lineplot(x=datetimes, y=time_series, ax=ax, color=color, label=labels[i])
        else:
            cvds = (time_series[0] <= y) & (time_series[1] >= y)
            ax.fill_between(datetimes, 
                            np.clip(time_series[0], minval_ax, maxval_ax), 
                            np.clip(time_series[1], minval_ax, maxval_ax), 
    ...
    # Add legend for color-method mapping in a blank area
    ax.legend(loc="center left", bbox_to_anchor=(0, 0.3), fontsize=10)

    sns.despine(ax=ax)  # Despine the top and right axes

    if inset:
        # Define the inset ax in the specified location
        axins = ax.inset_axes([0.6, 0.05, 0.4, 0.4]) if window_loc == 'lower right' else ax.inset_axes([0.6, 0.6, 0.4, 0.4]) if window_loc == 'upper right' else ax.inset_axes([0.05, 0.6, 0.4, 0.4])
        ...
        for i, time_series in enumerate(ts_list_finite):
            color = colors[i]
            if not sets:
                sns.lineplot(x=datetimes[window_start:window_end], 
                             y=time_series[window_start:window_end], 
                             ax=axins, color=color)
            else:
                axins.fill_between(datetimes[window_start:window_end], 
                                   np.clip(time_series[0][window_start:window_end], 
                                           minval_ax, maxval_ax), 
                                   np.clip(time_series[1][window_start:window_end], 
                                           minval_ax, maxval_ax), 
        ...
        # Customize the inset box
        box_color = "#dcd9d9"
        for axis in ['top', 'bottom', 'left', 'right']:
            axins.spines[axis].set_linewidth(2)
            axins.spines[axis].set_color(box_color)

        # Connect the inset axes to the main axes
        mark_inset(ax, axins, loc1=1, loc2=2, fc="none", ec=box_color, lw=2)
    ...
    # Set ymin and ymax for the main axes
    ax.set_ylim(minval_ax - 0.1 * np.abs(minval_ax), maxval_ax + 0.1 * np.abs(maxval_ax))
